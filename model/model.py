...
import numpy as np
from model.PLI import Graph
from torchdiffeq import odeint
from model.STODE6 import STODE
from model.crosstransformer_only import Cross
from model.selftransformer_temporal_position2 import Selftrans
...
class EncoderLayer(nn.Module):
    def __init__(self, nodes=62, seq_len=200, hidden_channels=64, h_num=4, dropout=0.0):
        ...
        self.adj = Graph(freqrange=(1, 51), srate=200, TH=0.15, init_alpha=2.0)

    def set_PLI(self, xs, PLI):
        self.x0 = xs.clone().detach()
        self.x1 = PLI.clone().detach()
    ...
    def forward(self, inputs):
        ...
        out2, adj_xs, adj_PLI = self.stodeg(out2, adj)
        ...
        return out3, out4, adj_xs, adj_PLI
...
class GODE(nn.Module):
    ...
    def forward(self, input):
        ...
        x, out4, adj_xs, adj_PLI = self.encoder_layer(input)
        x = self.linear(x)
        return  x
    def get_last_layer(self):
        return self.linear.get_last_layer()
